...
class Funnel:
    ...
    def get_training_data(self, r: bool = True, save: bool = False, rec_modes: list = RECMODES, subset: float = 1, filter: str = None, remove_noise: float = 0.95):
        ...
        # save settings to object for file output
        self.rm = rec_modes
        self.filter = filter
        self.subset = subset
        self.nr = remove_noise
        ...
        for rmidx, rm in enumerate(rec_modes):
            ...
            if save: 
                ...
                self._save_output(data=X, labels=y, rm=rm)

            # save return values in dict
            if r: 
                X_ret[rm] = X
                y_ret[rm] = y
    ...
    def _save_output(self, data = "", labels = "", rm = ""):
        ...
        # save files
        if data != "":
            np.savez(self.outputpath + "training_data" + "_" + str(rm) + "_" + str(self.filter) + "_subset_" + str(self.subset) + "_nrm_" + str(self.nr) + ".npz", data)
            self.logger.info(f'Data was saved under {self.outputpath}')
        else:
            self.logger.warning('No data to save!')

        if labels != "":
            np.savez(self.outputpath + "training_labels" + "_" + str(rm) + "_" + str(self.filter) + "_subset_" + str(self.subset) + "_nrm_" + str(self.nr) + ".npz", labels)
